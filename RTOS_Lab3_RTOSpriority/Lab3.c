...
int Testmain7(void){      // Testmain7  Lab 3
  ...
  OS_AddPeriodicThread(&Signal2,(1111*TIME_1MS)/1000,1);  // 1.111 ms, lower priority
	...
	//OS_AddPeriodicThread(&Signal1,(2400*TIME_1MS)/1000,0);   // 0.799 ms, higher priority
  //OS_AddPeriodicThread(&Signal2,(2500*TIME_1MS)/1000,1);  // 1.111 ms, lower priority
  NumCreated = 0 ;
  ...
  OS_Launch(TIME_2MS);  // 1ms, doesn't return, interrupts enabled in here
  return 0;             // this never executes
}


int Testmain8(void){  // Testmain2
  OS_Init();          // initialize, disable interrupts
  PortD_Init();       // profile user threads
  NumCreated = 0 ;
  NumCreated += OS_AddThread(&Thread1b,128,0); 
...
void Thread9b(void){
	NumCreated += OS_AddThread(&Thread9a,128,0); 
  Count2 = 0;          
  for(;;){
    //PD0 ^= 0x01;       // heartbeat
    Count2++;
...
int Testmain9(void){  // Testmain2
  OS_Init();          // initialize, disable interrupts
  PortD_Init();       // profile user threads
  NumCreated = 0 ;
  NumCreated += OS_AddThread(&Thread9b,128,1);
  // Count1 Count2 Count3 should be equal on average
  // counts are larger than Testmain1
...
int Testmain10(void){  // Testmain2
  OS_Init();          // initialize, disable interrupts
  PortD_Init();       // profile user threads
  NumCreated = 0 ;
  NumCreated += OS_AddThread(&Thread10a,128,1);
  // Count1 Count2 Count3 should be equal on average
  // counts are larger than Testmain1
...
int main(void) { 			// main 
	...
	//Testmain5();			// works now
	//Testmain6();			// works now
	//Testmain7();			// kind of works
	//TestmainFIFO();		// works now
	//Testmain8();			// works now
	//Testmain9();			// OS_Addthread needs added preemption ability
	//Testmain10();			// works now
  realmain();				// works now
}
