...
DRESULT eDisk_ReadBlock(
	...
	// Lab 4 used synchronization, but Lab 5 moved syncronization to eFile level
	//OS_bWait(&LCDFree);
	int errCode = eDisk_Read(0,buff,sector,1);
	//OS_bSignal(&LCDFree);
  return errCode;
}
...
DRESULT eDisk_WriteBlock (
	...
	// Lab 4 used synchronization, but Lab 5 moved syncronization to eFile level
	//OS_bWait(&LCDFree);
	int errCode = eDisk_Write(0,buff,sector,1);  // 1 block
  //OS_bSignal(&LCDFree);
	return errCode;
}
