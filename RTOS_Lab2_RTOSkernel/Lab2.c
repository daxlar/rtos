...
int realmain(void){     // realmain
	PLL_Init(Bus80MHz);		// added by merakian, interpreter needs this
...
int Testmain3(void){  // Testmain3
  ...
  OS_ClearMsTime();		 // ADDED for sleeping 
  OS_Launch(TIME_2MS); // doesn't return, interrupts enabled in here
  return 0;            // this never executes
}
...
int Testmain4(void){   // Testmain4
  ...
  NumCreated += OS_AddThread(&Thread4d,128,0); 
	OS_ClearMsTime();
  OS_Launch(TIME_2MS); // doesn't return, interrupts enabled2  in here
  return 0;            // this never executes
}
...
int main(void) { 			// main 
	//Testmain1(); 			// works now
	Testmain2();			// works now
	//TestmainCS();			// works now
	//Testmain3();			// works now
	//Testmain4();			// button debouncing not completely working
	//Testmain5();			// works now
	//TestmainFIFO();		// works now
	//realmain();				// losing a lot of data for some reason
}
