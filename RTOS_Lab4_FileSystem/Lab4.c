...
void ButtonWork(void){
  PD1 ^= 0x02;
  ST7735_Message(1,1,"NumCreated =",NumCreated); 
  PD1 ^= 0x02;
...
void SW2Push(void){
	...
	// sanity check to make sure file are persistent
	if(NumCreated > 20){
		eFile_Unmount();
	}
}
...
BYTE measurementDataBuffer[512];

void measurements(void){
	// measure performance without filesystems
	// measure time of reads and writes
	while(1){
			int dummyOperation = 5;
			PD0 ^= 0x01;
			eDisk_WriteBlock(measurementDataBuffer, 100);
			PD0 ^= 0x01;
			dummyOperation++;
	}
}
...
int realmain(void){        // lab 4 real main
  ...
  NumCreated += OS_AddThread(&Init,128,0);  // init process, run first
	//NumCreated += OS_AddThread(&measurements, 128, 0);
