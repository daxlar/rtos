...
int realmain(void){ // realmain
  ...
  CAN0_Open(RCV_ID,XMT_ID);   // CAN initialization
...
const char Fetch[] = "GET /data/2.5/weather?q=Austin&APPID=de826b6089682b5c47db1902e9c74c46 HTTP/1.1\r\nHost:api.openweathermap.org\r\n\r\n";
...
void TelnetClientTask(void){
	
	UART_OutString("Type 'ext_tnt' to exit telnet");
	UART_OutString("\n\r");
	
	char rpcCommandBuffer[12];
	while(1){
		UART_OutString("type in command:");
		UART_InString(rpcCommandBuffer, 11);
		if(strcmp(rpcCommandBuffer, "ext_tnt") == 0){
			break;
		}
		
		rpcCommandBuffer[7] = '\r';
		rpcCommandBuffer[8] = '\n';
		rpcCommandBuffer[9] = '\0';
		ESP8266_Send(rpcCommandBuffer);
	}
...
void serverClientAbitrationTask(void){
	Interpreter();
	OS_Kill();
}

void WebServer2(void){
  // Initialize and bring up Wifi adapter  
  if(!ESP8266_Init(true,false)) {  // verbose rx echo on UART for debugging
    ST7735_DrawString(0,1,"No Wifi adapter",ST7735_YELLOW); 
    OS_Kill();
  ...
  ST7735_DrawString(0,1,"Wifi connected",ST7735_GREEN);
	...
	// launch main thread interpreter
	NumCreated += OS_AddThread(&serverClientAbitrationTask, 128, 2);
	
	while(serverClientStatus == -1){
		// wait until a selection is made
	}
	
	// 1 is server
	if(serverClientStatus == 1){
		// telnet test
		if(!ESP8266_StartServer(7777,600)) {  // port 23, 5min timeout
			ST7735_DrawString(0,2,"Server failure",ST7735_YELLOW); 
			OS_Kill();
		}  
		ST7735_DrawString(0,2,"Server started",ST7735_GREEN);
		...
		while(1) {
			...
			// Launch thread with higher priority to serve request
			if(OS_AddThread(&TelnetTask,128,1)) NumCreated++;
	...
	// 0 is client
	if(serverClientStatus == 0){
		
		// this is yoon's tm4c's local IP address
		ESP8266_MakeTCPConnection("192.168.0.24", 7777, 0);
		
		// Launch thread with higher priority to serve request
    if(OS_AddThread(&TelnetClientTask,128,1)) NumCreated++;
...
int realmain2(void){
  ...
  NumCreated = 0 ;
  NumCreated += OS_AddThread(&Idle,128,3); 
  NumCreated += OS_AddThread(&WebServer2,128,2); 
...
int main(void) { 			// main
	//Testmain0();			//works by default	
	//Testmain1();			//what does this even test for?
	//Testmain2();			//works now
	//Testmain3();			//works now
  //realmain();				//???
	realmain2();
