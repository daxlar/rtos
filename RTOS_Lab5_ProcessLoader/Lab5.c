...
int realmain(void){ // realmain
  ...
  PortD_Init();     // debugging profile 
	PortF_Init(); 	// have to comment out OS_AddSW1Task and OS_AddSW2Task, weird interaction with initializing Port F
...
int main(void) { 			// main
	...
	//Testmain1();				// works now, display is wonky af
	//Testmain2();				// works now, display is still wonky
	//Testmain3();				// works now, display is still wonky
	
	// change the LCDFree placement in edisk, it's used twice in edisk and then in the new eFile
  realmain();
}
